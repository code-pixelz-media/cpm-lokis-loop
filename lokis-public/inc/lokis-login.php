    ...
    function cpm_login_redirect($redirect_to, $request, $user)
    {
        if (isset($user->roles) && is_array($user->roles)) {
            if (in_array('administrator', $user->roles)) {
                ...
                // $redirect_to = home_url('/instructions');

            } else if (in_array('host', $user->roles)) {
                $custom_dashboard_url = home_url('/instructions');
...
// function redirect_admin_on_login($redirect_to, $request, $user)
// {
//     // Check if the user has the 'administrator' role
//     if (isset($user->roles) && is_array($user->roles) && in_array('administrator', $user->roles)) {
//         $redirect_to = site_url(); // Redirect administrators to the home page
//     }
//     return $redirect_to;
// }

// Add the filter for the login redirect
// add_filter('login_redirect', 'redirect_admin_on_login', 20, 3);
